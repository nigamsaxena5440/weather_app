...
class HomeScreen extends StatelessWidget {
  ...
  Color _getDarkVariant(String? condition) {
    if (condition == null) return const Color(0xFFB35E00);
    final text = condition.toLowerCase();
    if (text.contains("sunny") || text.contains("clear")) {
                              ...
                              children: [
                                Image.network(
                                  current?.condition?.icon
                                          ?.replaceFirst('//', 'https://') ??
                                      '',
                                  width: w * (isTablet ? 0.18 : 0.28),
                                  height: w * (isTablet ? 0.18 : 0.28),
                                  fit: BoxFit.contain,
                                ),
                                Text(
                                  "${current?.tempC?.round() ?? 0}°",
                                  style: TextStyle(
                                    fontSize: w * (isTablet ? 0.14 : 0.22),
                                    color: Colors.white,
      ...
      child: Row(
        children: [
          Container(
            ...
            child: Icon(icon, color: Colors.white, size: isTablet ? 22 : 17),
          ),
